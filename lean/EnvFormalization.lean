...
theorem getAt?_replaceAt_same {α : Type} {xs : List α} {n : Nat} {value : α}
    (h : n < xs.length) :
    getAt? (replaceAt xs n value) n = some value := by
  ...
  | cons x xs ih =>
      cases n with
      | zero =>
          simp [replaceAt, getAt?]
      | succ n =>
          simp at h
          simp [replaceAt, getAt?, ih h]

theorem currentRoom_setCurrentRoom (w : WorldState) (room : RoomState)
    (h : w.currentRoomIdx < w.rooms.length) :
    currentRoom (setCurrentRoom w room) = room := by
  unfold currentRoom setCurrentRoom
  rw [getAt?_replaceAt_same h]
...
theorem Jenv_move_in_bounds {w : WorldState} {d : Direction}
    (hPlayer : InBounds w.player) :
    InBounds (basicMove w d).player := by
  by_cases hOcc : canOccupy (currentRoom w) (facingTile w.player d) = false
  · rw [blocked_basicMove_keeps_player hOcc]
    exact hPlayer
  · have hOccTrue : canOccupy (currentRoom w) (facingTile w.player d) = true := by
      cases hVal : canOccupy (currentRoom w) (facingTile w.player d) <;> simp [hVal] at hOcc ⊢
    exact free_basicMove_stays_in_bounds hOccTrue
...
theorem Tenv_interact_range (w : WorldState) (d1 d2 : Direction)
    (hInteract :
      (currentRoom w).chests.find? (fun c => c.isVisible && !c.isOpen && adjacent w.player c.pos) ≠ none ∨
      (currentRoom w).npcs.find? (fun n => adjacent w.player n.pos) ≠ none ∨
      (currentRoom w).switches.find? (fun sw => adjacent w.player sw.pos) ≠ none) :
    (interactStep { w with facing := d1 }).snd = false ∧
    (interactStep { w with facing := d2 }).snd = false := by
  rcases hInteract with hChest | hRest
  · cases hChestFind : (currentRoom w).chests.find? (fun c => c.isVisible && !c.isOpen && adjacent w.player c.pos) with
    | none => contradiction
    | some chest =>
        have hChestFind1 :
            (currentRoom { w with facing := d1 }).chests.find? (fun c => c.isVisible && !c.isOpen && adjacent w.player c.pos) = some chest := by
          simpa [currentRoom] using hChestFind
        have hChestFind2 :
            (currentRoom { w with facing := d2 }).chests.find? (fun c => c.isVisible && !c.isOpen && adjacent w.player c.pos) = some chest := by
          simpa [currentRoom] using hChestFind
        simp [interactStep, hChestFind1, hChestFind2]
  · rcases hRest with hNpc | hSwitch
    · cases hChestFind : (currentRoom w).chests.find? (fun c => c.isVisible && !c.isOpen && adjacent w.player c.pos) with
      | some chest =>
          have hChestFind1 :
              (currentRoom { w with facing := d1 }).chests.find? (fun c => c.isVisible && !c.isOpen && adjacent w.player c.pos) = some chest := by
          ...
          simp [interactStep, hChestFind1, hChestFind2]
      | none =>
          cases hNpcFind : (currentRoom w).npcs.find? (fun n => adjacent w.player n.pos) with
          | none => contradiction
          | some npc =>
              have hChestFind1 :
                  (currentRoom { w with facing := d1 }).chests.find? (fun c => c.isVisible && !c.isOpen && adjacent w.player c.pos) = none := by
                simpa [currentRoom] using hChestFind
              have hChestFind2 :
                  (currentRoom { w with facing := d2 }).chests.find? (fun c => c.isVisible && !c.isOpen && adjacent w.player c.pos) = none := by
                simpa [currentRoom] using hChestFind
              have hNpcFind1 :
                  (currentRoom { w with facing := d1 }).npcs.find? (fun n => adjacent w.player n.pos) = some npc := by
                simpa [currentRoom] using hNpcFind
              have hNpcFind2 :
                  (currentRoom { w with facing := d2 }).npcs.find? (fun n => adjacent w.player n.pos) = some npc := by
                simpa [currentRoom] using hNpcFind
              simp [interactStep, hChestFind1, hChestFind2, hNpcFind1, hNpcFind2]
    · cases hChestFind : (currentRoom w).chests.find? (fun c => c.isVisible && !c.isOpen && adjacent w.player c.pos) with
      | some chest =>
          have hChestFind1 :
              (currentRoom { w with facing := d1 }).chests.find? (fun c => c.isVisible && !c.isOpen && adjacent w.player c.pos) = some chest := by
      ...
      | none =>
          ...
          | some npc =>
              ...
              simp [interactStep, hChestFind1, hChestFind2, hNpcFind1, hNpcFind2]
          | none =>
              cases hSwitchFind : (currentRoom w).switches.find? (fun sw => adjacent w.player sw.pos) with
              | none => contradiction
              | some sw =>
                  have hChestFind1 :
                      (currentRoom { w with facing := d1 }).chests.find? (fun c => c.isVisible && !c.isOpen && adjacent w.player c.pos) = none := by
                  ...
                  have hChestFind2 :
                    ...
                    simpa [currentRoom] using hChestFind
                  have hNpcFind1 :
                      (currentRoom { w with facing := d1 }).npcs.find? (fun n => adjacent w.player n.pos) = none := by
                    simpa [currentRoom] using hNpcFind
                  have hNpcFind2 :
                      (currentRoom { w with facing := d2 }).npcs.find? (fun n => adjacent w.player n.pos) = none := by
                    simpa [currentRoom] using hNpcFind
                  have hSwitchFind1 :
                      (currentRoom { w with facing := d1 }).switches.find? (fun s => adjacent w.player s.pos) = some sw := by
                    simpa [currentRoom] using hSwitchFind
                  have hSwitchFind2 :
                      (currentRoom { w with facing := d2 }).switches.find? (fun s => adjacent w.player s.pos) = some sw := by
                    simpa [currentRoom] using hSwitchFind
                  simp [interactStep, hChestFind1, hChestFind2, hNpcFind1, hNpcFind2, hSwitchFind1, hSwitchFind2]

theorem Tenv_sword_hit (w : WorldState) (monster : Monster)
    (hChest : (currentRoom w).chests.find? (fun c => c.isVisible && !c.isOpen && adjacent w.player c.pos) = none)
    (hNpc : (currentRoom w).npcs.find? (fun n => adjacent w.player n.pos) = none)
    (hSwitch : (currentRoom w).switches.find? (fun sw => adjacent w.player sw.pos) = none)
    (hSword : hasEquippedSword w = true)
    (hMonster : (currentRoom w).monsters.find? (fun m => adjacent w.player m.pos && facingTile w.player w.facing == m.pos) = some monster)
    (hHp : 1 < monster.hp) :
    interactStep w =
      ((setCurrentRoom (startAction w "sword" w.facing 6)
          { (currentRoom w) with
              monsters :=
                setMonsterStunById
                  (reduceMonsterHpById (currentRoom w).monsters monster.monsterId)
                  monster.monsterId 60 }),
        true) := by
  simp [interactStep, hChest, hNpc, hSwitch, hSword, hMonster, Nat.not_le_of_gt hHp]

theorem Tenv_sword_kill (w : WorldState) (monster : Monster)
    (hChest : (currentRoom w).chests.find? (fun c => c.isVisible && !c.isOpen && adjacent w.player c.pos) = none)
    (hNpc : (currentRoom w).npcs.find? (fun n => adjacent w.player n.pos) = none)
    (hSwitch : (currentRoom w).switches.find? (fun sw => adjacent w.player sw.pos) = none)
    (hSword : hasEquippedSword w = true)
    ...
    (hHp : monster.hp ≤ 1) :
    interactStep w =
      (onMonsterKilled (startAction w "sword" w.facing 6) monster, true) := by
  simp [interactStep, hChest, hNpc, hSwitch, hSword, hMonster, hHp]

theorem Tenv_sword_hit_precedes_contact (w : WorldState)
    (hLock : w.controlLockStepsRemaining = 0)
    (hAlive : w.health ≠ 0) :
    step w .interactA =
      finalizePlayerActionState
        (postActionResolve (advancePlayerActionState w) (interactStep (advancePlayerActionState w)).fst .interactA)
        (interactStep (advancePlayerActionState w)).snd := by
  simp [step, hLock, hAlive, actionStep]

theorem env_damage_contact (w : WorldState) (monster : Monster)
    (hMonster : (currentRoom w).monsters.find? (fun m => m.pos == w.player && m.stunTicksRemaining = 0) = some monster)
    (hShield : shieldActive w = false) :
    (resolveMonsterContact w).health = w.health - monster.damage := by
  simp [resolveMonsterContact, hMonster, hShield, setCurrentRoom]

theorem env_damage_contact_agent_dead (w : WorldState) (monster : Monster)
    (hMonster : (currentRoom w).monsters.find? (fun m => m.pos == w.player && m.stunTicksRemaining = 0) = some monster)
    (hShield : shieldActive w = false)
    (hLethal : w.health ≤ monster.damage) :
    (resolveMonsterContact w).health = 0 := by
  rw [env_damage_contact w monster hMonster hShield]
  exact Nat.sub_eq_zero_of_le hLethal

theorem env_spike_damage_exact (w : WorldState) (trap : Trap) :
    (resolveSpikeTrap w trap).health = w.health - trap.damage := by
  simp [resolveSpikeTrap]

theorem env_abyss_damage_exact (w : WorldState) (trap : Trap) (previous : Position) :
    (resolveAbyssTrap w trap previous).health = w.health - trap.damage := by
  by_cases hs : trap.singleUse
  · by_cases hh : w.health - trap.damage = 0
    · simp [resolveAbyssTrap, hs, hh]
    · simp [resolveAbyssTrap, hs, hh]
...
theorem env_agent_dead_absorbing (w : WorldState) (a : Action)
    (hLock : w.controlLockStepsRemaining = 0)
    (hDead : w.health = 0) :
    step w a = w := by
  simp [step, hLock, hDead]

theorem env_chest_open_effect (w : WorldState) (chest : Chest)
    (hFind : (currentRoom w).chests.find? (fun c => c.isVisible && !c.isOpen && adjacent w.player c.pos) = some chest) :
    interactStep w =
      (applyLoot
        (setCurrentRoom w
        ...
        chest.loot,
      false) := by
  simp [interactStep, hFind]

theorem Tenv_button_toggle_effect (w : WorldState) (sw : Switch)
    (hChest : (currentRoom w).chests.find? (fun c => c.isVisible && !c.isOpen && adjacent w.player c.pos) = none)
    (hNpc : (currentRoom w).npcs.find? (fun n => adjacent w.player n.pos) = none)
    (hSwitch : (currentRoom w).switches.find? (fun s => adjacent w.player s.pos) = some sw) :
    interactStep w = (applySwitchToggle w sw, false) := by
  simp [interactStep, hChest, hNpc, hSwitch]

theorem env_button_press_effect (w _previous : WorldState) (button : Button)
    (_hButton : buttonAt? (currentRoom w) w.player = some button)
    (hIdx : w.currentRoomIdx < w.rooms.length)
    (_hPressed : button.isPressed = false) :
    (currentRoom
      (setCurrentRoom w
        { (currentRoom w) with buttons := pressButtonById (currentRoom w).buttons button.buttonId })).buttons =
      pressButtonById (currentRoom w).buttons button.buttonId := by
  rw [currentRoom_setCurrentRoom _ _ hIdx]
