...
### Environment interaction and damage theorems

The following theorems cover the required environment interaction and damage
properties:

- `Jenv_move_in_bounds`
  any move action keeps the player within map bounds (engine clamp semantics)
- `env_move_blocked_stationary`
  if the target tile is blocked or out of bounds, the move does not change
  player position (AABB collision semantics)
- `Tenv_interact_range`
  `BUTTON_A` interaction applies to objects within Manhattan distance ≤ 1,
  independent of the player's facing direction
- `Tenv_sword_hit`
  sword swing damages a monster in the facing tile when HP > 1, reducing HP
  by 1 and stunning for 60 ticks; `Tenv_sword_kill` covers the lethal case
  (HP ≤ 1 triggers `onMonsterKilled`)
- `Tenv_sword_hit_precedes_contact`
  within `step`, sword resolution (`interactStep`) is applied before monster
  contact resolution (`postActionResolve`), matching the Python engine ordering
- `env_damage_contact`
  contact with an unstunned monster without an active shield reduces player HP
  by the monster's damage; `env_damage_contact_agent_dead` shows lethal contact
  sets HP to 0; `env_agent_dead_absorbing` shows HP = 0 is an absorbing state
- `env_spike_damage_exact` / `env_abyss_damage_exact`
  spike and abyss traps reduce HP by exactly `trap.damage`
- `env_chest_open_effect`
  interacting with an adjacent unopened chest sets it to opened and applies
  its loot to the player state
- `Tenv_button_toggle_effect`
  interacting with an adjacent switch triggers `applySwitchToggle`, cycling
  the target dynamic object's state
- `env_button_press_effect`
  standing on an unpressed button updates the room's button list via
  `pressButtonById`, changing the corresponding mechanism state
