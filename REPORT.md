...
|---|---|---|---|---|
| task_1 取钥匙开门 | **100%** | 279 | 279 | **步数 = 参照最优** |
| task_2 杀怪+条件门 | **100%** | 193 | 197 | **反超参照** |
| task_3 三房任务链 | **100%** | 668 | 582 | 由 1449 大幅提速（上限 1500） |
| task_4 旋桥+全宝箱 | **100%** | 1643 | 1481 | 感知修复后误感知交互绝迹；轨迹与 v3b 同构（各一次盲探+两次必需拨杆），+62 为目标排序噪声 |
| task_5 多房+隐藏扣血 | **100%** | 1118 | 1118 | **与参照持平**（HP 记账 + west-key 修复后） |

Oracle 参照 = 同版本符号层用完美感知（`OracleGrounding`）的实跑步数。
以上为 2026-07-08 感知层 v4b 复测（v3b 时 task_4 为 1581，其余同）；
...
- 感知在 3000 个独立布局上训练、在完全未见布局上 96% 整格全对——直面
  "最终测评可能变布局"的考察点；
- **task_4 三变体 VLM 全通**（v4b）：object_shift 1677（v3b/v4 均 2000 截断，
  根因为课程房记忆先验，见 §2.7）、switch_shift 1795→1583、
  final_chest_shift 1927→1659；oracle 参照 1375/1325/1337。
