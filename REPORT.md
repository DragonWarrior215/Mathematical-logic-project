...
派生）。`hp_estimate` 血量记账走 **reward 反馈**（与 §2.4 撞墙检测同一合规
先例）：`hp_loss` 信号覆盖怪物命中与 task_5 的每 200 步隐藏扣血，`agent_healed`
信号记回血（训练图 +1/次）并累加 `heal_events` 供 task_5 阶段机判定"已回血"；
信号缺失时回退到纯扣血 tick 模型。
...
| 任务 | 成功率 | 步数 | Oracle 参照 | 备注 |
|---|---|---|---|---|
| task_1 取钥匙开门 | **100%** | 280 | 280 | **= 参照** |
| task_2 杀怪+条件门 | **100%** | 176 | 176 | **= 参照** |
| task_3 三房任务链 | **100%** | 541 | 541 | **= 参照**（由 1449 大幅提速） |
| task_4 旋桥+全宝箱 | **100%** | 1514 | 1514 | **= 参照**，误感知交互绝迹 |
| task_5 多房+隐藏扣血 | **100%** | 1042 | 1042 | **= 参照**，怪物零接触 |

Oracle 参照 = 同版本符号层用完美感知（`OracleGrounding`）的实跑步数。
**五关 VLM 与 oracle 步数逐位一致——感知层开销归零**：v4b 感知 +
行为闩锁三件套（opened_exits / opened_chests / pressed_buttons，
§6.1）+ 出口贴边直接转场 + 显式路线成本模型的叠加结果
（2026-07-08 终测）。历史演进：v3b 时 task_4=1581、task_5=1118；
修复前基线与逐项归因见 `test_generalization/test2_report.md`。
...
- 感知在 3000 个独立布局上训练、在完全未见布局上 96% 整格全对——直面
  "最终测评可能变布局"的考察点；
- **15 个地图变体 VLM 全量评测 13/15**（2026-07-08 终测）：task_1–4 的
